...
int main() {
	...
	string text_case_0[quantity_languages][4] = {
		//English language -- default
		{
			...
			switch (main_selected_item) {
				...
				case 0: {
					...
					do {
						...
						cin >> len_row_A;
					} while (len_row_A < 1 || len_row_A > 10);
					...
					} while (len_column_A_row_B < 1 || len_column_A_row_B > 10);
					...
					do {
						...
						cin >> len_column_B;
					} while (len_column_B < 1 || len_column_B > 10);

					getchar();

					int** matrix_a = get_matrix(len_row_A, len_column_A_row_B);
					int** matrix_b = get_matrix(len_column_A_row_B, \
						len_column_B);
...
int **get_matrix(int iterim_row, int iterim_column) {
	...
	int **iterim_array = (int **) malloc(sizeof(int) * iterim_row);

	for (int u = 0; u < iterim_row; u++) {
		iterim_array[u] = (int *) malloc(sizeof(int) * iterim_column);
	}
