...
var
  chng, clm, shw, n2, sgn, i, k, x, y, badrow: byte;
  min, max: string;
...
begin
  ...
  if round(nn / chk) mod clm = 0 then badrow := round(d / chk)
    else badrow := round(d / chk - 1);
  ...
  {write('':x, '╚');
  for i := 1 to (h * clm - n2 - 1) * 13 - 1 do 
    if i mod 13 <> 0 then 
      write('═') 
    else if i <> clm * 13 then write('╧');
  if n2 mod clm = 0 then  write('╝')
    else write('╛');}
