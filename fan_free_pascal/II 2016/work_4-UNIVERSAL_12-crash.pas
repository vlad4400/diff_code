...
var
  chng, shw, clm, n2, sgn, k, x, y, i, badrow, s, kres, j: byte;
  min, max: string;
  n1, n1c, chk, nn, d, h, res, v, iu: longint;
  rlim: real;
  //iu - iconic unit;
  //flagEnded: boolean;
  //priz: integer;
...
begin
        ...
        begin
          ...
          //begin
            ...
            begin
              ...
              //write(n1,' ');
              //write((n1 + chk) * iu,' ', n2 * iu);
              if (n1 + chk) * iu > n2 * iu  then n1 := n2;
            end
            else n1 := n2;
          //end;
          {if sgn = 0  then 
  ...
  begin
    ...
    begin
      ...
      begin
        //write(kres);
        res := n1 + (i - 1) * d;//write(i,' ',v+2);
        for j := 1 to kres do
        begin
        //write(i,v+res);
        res := n1 + (i - 1) * d;
        if i > v {or (badrow = d / chk)} then
          res := res - chk * (i - v - 1) - chk * (kres - 1);//write(i - v - 1);
        end;
        //if i > v then res := res - chk * (i - v - 1);
        if (v = clm) and (badrow <> nn / chk) 
          then res := res - chk * (i - 1);
        //write(v);
          //if badrow <= d / chk then res := n2 + 1;write(badrow,d/chk);
          //write(res + d * (clm - i),n2);
          //if res + (d - chk) * (clm - i) >= n2 then res := n2 + 1;//l
          //write(res);
        if (s = badrow + 1) and (i >= v + 1) 
          then res := n2 + 1 * iu;
        if (s = badrow) and (v = clm) 
          then res := n2 + 1 * iu;
        //write(s,' ',badrow + 1,' ',v,' ',clm);
        //if (v = clm) and (badrow <> nn / chk) then res := n2 + 1;
        //write(' s:',s,' badrow:',badrow);write(' n2+1:',n2+1);
        ...
        //write(res,' ',n2 * iu);
        if {(sgn = 1) and} (res * iu <= n2 * iu) then 
        begin
          case res of
        ...
        //if i >= v + 2 then res := res + chk;
        if i <> clm then write(' │') 
        else if res * iu <= n2 * iu then write(' ║'); //case sgn of
            //1: if res <= n2 then write(' ║');
            //2: if res >= n2 then write(' ║');
          //end;
        //write(' i:',i,' s+1:',s+1,' n1c:',n1c,' n1+chk:',n1+chk);
        //if (i = s + 1) {and (n1c = n1 - chk)} then 
        //begin
        //d := d - chk; write(d)
        //h := h - chk;
        //end;
