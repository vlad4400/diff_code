...
var
  chng, clm, shw, n2, sgn, i, j, k, n: byte;
  min, max: string;
  n1, chk, nn, d, h1, h2, res, iu, prom: longint;
  //iu - iconic unit;
...
begin
  ...
  //!!!
  if (n1 = 0) or (n2 = 0) then nn := (n2 - n1) * iu + 1
  else nn := (n2 - n1) * iu;
  
  while frac(nn / chk * iu) <> 0 do
  begin
    n2 := n2 - 1 * iu;
    if (n1 = 0) or (n2 = 0) then nn := (n2 - n1) * iu + 1
    else nn := (n2 - n1) * iu;
