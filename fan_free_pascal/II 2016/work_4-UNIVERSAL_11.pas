...
var
  chng, shw, clm, n2, sgn, k, x, y, i, badrow, s: byte;
  min, max: string;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if i > v + 1 {or (badrow = d / chk)} then
          res := res - chk * (i - v - 1);
        if (v = clm) and (badrow <> nn / chk) then res := res - chk * (i - 1);
        //write(v);
          //if badrow <= d / chk then res := n2 + 1;write(badrow,d/chk);
          //write(res + d * (clm - i),n2);
          //if res + (d - chk) * (clm - i) >= n2 then res := n2 + 1;//l
          //write(res);
        if (s = badrow + 1) and (i >= v + 1) then res := n2 + 1 * iu;
        if (s = badrow) and (v = clm) then res := n2 + 1 * iu;
